...
| Image | 版本 |
|-------|------|
| `coolguazi/network-dashboard-base:v2.21.1` | 主應用 |
| `coolguazi/netora-mock-server:v2.21.1` | Mock API（僅 Mock 模式需要） |

掃描通過後更新 `.env`：

```ini
APP_IMAGE=registry.company.com/netora/network-dashboard-base:v2.20.2
MOCK_IMAGE=registry.company.com/netora/netora-mock-server:v2.21.1   # Mock 模式才需要
```
...
| `coolguazi/netora-mock-server:v2.21.1` | Mock API（僅 Mock 模式） |
...
MOCK_IMAGE=registry.company.com/netora/netora-mock-server:v2.21.1
```
...
docker pull registry.company.com/netora/netora-mock-server:v2.21.1
