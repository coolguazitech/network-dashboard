"""
Parser for 'get_transceiver_nxos_fna' API.

Auto-generated skeleton by scripts/generate_parsers.py.
Fill in the parse() method logic.

API Source: get_transceiver_fna
Endpoint: http://localhost:8001/switch/get_transceiver/10.1.1.3
...
class GetTransceiverNxosFnaParser(BaseParser):
    """
    Parser for get_transceiver_nxos_fna API response.

    Example raw output from Mock-NXOS-Switch:
    ```
    Interface    Tx Power (dBm)    Rx Power (dBm)
    GE1/0/1           -2.5              -3.1
    GE1/0/2           -2.3              -3.0
    XGE1/0/25         -1.8              -2.5
    ```
    ...
    Example after filling in:
        ...
        class GetTransceiverNxosFnaParser(BaseParser[FanStatusData]):
            device_type = DeviceType.HPE
            command = "get_transceiver_nxos_fna"
...
# Register parser
parser_registry.register(GetTransceiverNxosFnaParser())