...
# Configuration - Updated to match database naming
CORE_SWITCHES = [f"SW-NEW-{i:03d}-CORE" for i in range(1, 3)]  # 2 CORE
AGG_SWITCHES = [f"SW-NEW-{i:03d}-AGG" for i in range(1, 9)]  # 8 AGG
EDGE_SWITCHES = {
    "EQP": [f"SW-NEW-{i:03d}-EQP" for i in range(1, 11)],   # 10 EQP
    "AMHS": [f"SW-NEW-{i:03d}-AMHS" for i in range(1, 5)],  # 4 AMHS
    "SNR": [f"SW-NEW-{i:03d}-SNR" for i in range(1, 6)],    # 5 SNR
    "OTHERS": [f"SW-NEW-{i:03d}-OTHERS" for i in range(1, 6)],  # 5 OTHERS
}
...
def get_uplink_neighbors(hostname: str) -> list[tuple[str, str, str]]:
    ...
    Returns list of (local_interface, neighbor_hostname, neighbor_interface) tuples.
    """
    neighbors = []

    # EDGE → AGG uplinks
    all_edge_switches = []
    for category in ["EQP", "AMHS", "SNR", "OTHERS"]:
        all_edge_switches.extend(EDGE_SWITCHES[category])
...
def generate_hpe_transceiver_output(hostname: str) -> str:
    ...
    # EDGE switches have uplinks on XGE1/0/51, XGE1/0/52
    # AGG switches have uplinks on HGE1/0/25, HGE1/0/26
    # Also include some access ports
...
def generate_hpe_neighbor_output(hostname: str) -> str:
    ...
    for local_if, neighbor_host, neighbor_if in neighbors:
        output_lines.append(f"""Local Interface {local_if} has 1 neighbor:
Neighbor Index   : 1
...
Management Address        : {_get_ip_for_hostname(neighbor_host)}
Expired Time              : 120
""")
...
def _generate_chassis_mac(hostname: str) -> str:
    ...
    # Simple hash to generate consistent MAC
    hash_val = sum(ord(c) for c in hostname)
    return f"00:1a:2b:{hash_val % 256:02x}:{hash_val // 256 % 256:02x}:{hash_val // 65536 % 256:02x}"


def _get_ip_for_hostname(hostname: str) -> str:
    """Get IP address for a hostname."""
    if hostname in CORE_SWITCHES:
        idx = CORE_SWITCHES.index(hostname) + 1
        return f"10.100.1.{idx}"
    elif hostname in AGG_SWITCHES:
        idx = AGG_SWITCHES.index(hostname) + 1
        return f"10.100.2.{idx}"
    else:
        # EDGE switch
        all_edge = []
        for category in ["EQP", "AMHS", "SNR", "OTHERS"]:
            all_edge.extend(EDGE_SWITCHES[category])
        if hostname in all_edge:
            idx = all_edge.index(hostname) + 1
            return f"10.100.3.{idx}"
    return "10.100.0.1"


def generate_hpe_version_output(version: str = TARGET_VERSION) -> str:
...
def generate_hpe_port_channel_output(hostname: str) -> str:
    ...
    if hostname in all_edge_switches:
        # EDGE switch - BAGG1 with XGE1/0/51, XGE1/0/52
...
"""
        ...
        # AGG switch - BAGG1 with HGE1/0/25, HGE1/0/26
        return """Aggregate Interface: Bridge-Aggregation1
Aggregation Mode: Dynamic
Loadsharing Type: Shar
  Port             Status   Weight
...
def generate_hpe_error_output(hostname: str) -> str:
    """Generate HPE Comware interface error counters (all zeros for baseline)."""
    all_edge_switches = []
    for category in ["EQP", "AMHS", "SNR", "OTHERS"]:
        all_edge_switches.extend(EDGE_SWITCHES[category])
    ...
    if hostname in all_edge_switches:
        # EDGE switch - uplinks and a few access ports
        for port in [51, 52]:
...
def generate_baseline_scenario() -> dict:
    """Generate baseline scenario with all 34 switches in healthy state."""
    scenario = {}

    # CORE switches
    for idx, hostname in enumerate(CORE_SWITCHES, start=1):
        scenario[hostname] = {
            "ip_address": f"10.100.1.{idx}",
            "vendor": "HPE",
            "platform": "Comware",
            "site": "t_site",
            "power": "on",
    ...
    # AGG switches
    for idx, hostname in enumerate(AGG_SWITCHES, start=1):
        scenario[hostname] = {
            "ip_address": f"10.100.2.{idx}",
            "vendor": "HPE",
            "platform": "Comware",
            "site": "t_site",
            "power": "on",
    ...
    edge_idx = 1
    for category in ["EQP", "AMHS", "SNR", "OTHERS"]:
        for hostname in EDGE_SWITCHES[category]:
            scenario[hostname] = {
                "ip_address": f"10.100.3.{edge_idx}",
                "vendor": "HPE",
                "platform": "Comware",
                "site": "t_site",
                "power": "on",
...
def generate_transceiver_failure_scenario() -> dict:
    ...
    # Inject transceiver failure on a few EDGE switches
    failed_switches = ["EQP-SW-01", "AMHS-SW-01", "SNR-SW-01"]

    for hostname in failed_switches:
        if hostname in scenario:
            # Modify transceiver output to show low RX power
            scenario[hostname]["transceiver_output"] = scenario[hostname]["transceiver_output"].replace(
                "Current RX Power(dBm)         : -3.15",
                "Current RX Power(dBm)         : -15.50"  # Below threshold
            )
...
def generate_uplink_down_scenario() -> dict:
    ...
    # Disconnect one uplink on an EDGE switch
    hostname = "EQP-SW-02"
    if hostname in scenario:
        # Remove one neighbor from LLDP output
...
def generate_version_mismatch_scenario() -> dict:
    ...
    wrong_version_devices = ["EQP-SW-03", "AMHS-SW-02"]

    for hostname in wrong_version_devices:
        if hostname in scenario:
            scenario[hostname]["version_output"] = generate_hpe_version_output("6635P05")
...
def generate_port_channel_degraded_scenario() -> dict:
    ...
    # Degrade Port-Channel on an EDGE switch
    hostname = "EQP-SW-04"
    if hostname in scenario:
        # Modify port-channel output to show one member down
        scenario[hostname]["port_channel_output"] = """Aggregate Interface: Bridge-Aggregation1
Aggregation Mode: Dynamic
Loadsharing Type: Shar
  Port             Status   Weight
  XGE1/0/51        S        1
...
def main():
    ...
    scenarios = {
        ...
        "factory_transceiver_failure.yaml": generate_transceiver_failure_scenario(),
        "factory_uplink_down.yaml": generate_uplink_down_scenario(),
        "factory_version_mismatch.yaml": generate_version_mismatch_scenario(),
        "factory_port_channel_degraded.yaml": generate_port_channel_degraded_scenario(),
    }
    ...
    for filename, scenario_data in scenarios.items():
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(f"# Generated scenario with 34 switches\n")
            f.write(f"# Topology: 2 CORE + 8 AGG + 24 EDGE\n\n")
